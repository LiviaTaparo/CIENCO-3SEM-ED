...
void merge(int vetor[TAM], int indiceEsquerdo, int meio, int indiceDireito)
{
    ...
    //passa os elementos do veotr principal para o segundo vetor auxiliar(direita)
    for(j = 0; j < n2; j++)
    {
        vetorDireito[j] = vetor[meio + 1 + j];
    }
    //resseta as variaveis
    i = 0;
    j = 0;
    k = indiceEsquerdo;
    while(i < n1 && j < n2)
    {
        ...
        if(vetorEsquerdo[i] <= vetorDireito[j])
        {
            ...
            //incrementa o auxiliar para passar a analise para os proximos valores
            //do valor auxiliar
            i++;
        }
        else
    ...
    //se faltarem alguns elemnetos do array esquerdo, passa eles para o array principal
    while(i < n1)
    {
        vetor[k] = vetorEsquerdo[i];
        i++;
    ...
    //se faltarem alguns elementos do array direito, passa ele para o array principal
    while(j < n2)
    {
        vetor[k] = vetorDireito[j];
        j++;
